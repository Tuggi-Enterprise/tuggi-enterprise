/**
 * Edge geo lookup for the base-market price. Reads Vercel's geo header and
 * returns the country code, or `null` when there is no usable one.
 *
 ...
 * literal, which is why the code above names none.
 *
...
/** ISO-3166-1 alpha-2, and nothing else: "XX", "" and "Brazil" are all `null`,
 *  which resolves to the store note rather than to a guess. */
const COUNTRY_CODE = /^[A-Za-z]{2}$/;

export function GET(request: Request): Response {
  const header = request.headers.get("x-vercel-ip-country")?.trim() ?? "";
  const country = COUNTRY_CODE.test(header) ? header.toUpperCase() : null;
